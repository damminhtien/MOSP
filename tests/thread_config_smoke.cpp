...
int main() {
    ...
    assert(sopmoa_sixteen.effective_threads == 16);
    ...
    assert(oversubscribe_message.find("effective=16") != std::string::npos);
    assert(oversubscribe_message.find("hardware=8") != std::string::npos);
    assert(oversubscribe_message.find("oversubscribing hardware") != std::string::npos);
