...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="mosp-cli-smoke-") as tmp_dir_str:
        ...
        assert_failed(run_case(binary, ["--summary-output", str(summary_path), "-m", str(map_a), str(map_b), "-a", "SOPMOA", "-n", "13", "-s", "0", "-t", "0"]), "SOPMOA supports at most 12 threads")
        assert_failed(run_case(binary, ["--summary-output", str(summary_path), "-m", str(map_a), str(map_b), "-a", "SOPMOA_bucket", "-n", "17", "-s", "0", "-t", "0"]), "SOPMOA_bucket supports at most 16 threads")
