...
inline SolverThreadConfig resolve_solver_thread_config(
    ...
    if (algorithm == "SOPMOA" && config.effective_threads > 12) {
        config.effective_threads = 12;
        config.capped = true;
        config.cap_reason = "SOPMOA max 12 threads";
    } else if (algorithm == "SOPMOA_bucket" && config.effective_threads > 16) {
        config.effective_threads = 16;
        config.capped = true;
        config.cap_reason = "SOPMOA_bucket max 16 threads";
    }
